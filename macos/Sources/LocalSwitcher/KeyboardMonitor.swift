...
let kLocalSwitcherEventMarker: Int64 = 0x52555300
...
private let rsLogQueue = DispatchQueue(label: "com.marko.localswitcher.log")

func rslog(_ msg: String) {
    // Thread-safe: читаем UserDefaults напрямую (без MainActor)
    guard UserDefaults.standard.bool(forKey: "com.marko.localswitcher.debugLog") else { return }
...
struct TriggerConfig {
    ...
    static var sharedShiftGesturesEnabled: Bool {
        let s = SettingsManager.shared
        return s.triggerKey == "shift" && s.switchHotkey == "shift"
    }
...
final class KeyboardMonitor: @unchecked Sendable {
    ...
    /// Последнее приложение, в котором наблюдался keyDown. Нужен для сброса
    /// незавершённого слова при программной смене фокуса без клика.
    private var inputBundleID: String?
    ...
    private var shiftGestures = ShiftGestureRecognizer()
    private var observedShiftSides: Set<ShiftSide> = []
    private var pendingShiftSingle: DispatchWorkItem?
    ...
    fileprivate func handleKeyDown(keyCode: UInt16, flags: CGEventFlags, char: Character? = nil) {
        ...
        if TriggerConfig.sharedShiftGesturesEnabled {
            ...
            performShiftActions(
                shiftGestures.nonShiftKeyDown(at: ProcessInfo.processInfo.systemUptime)
            )
        }
        triggerArmed = false
    ...
    fileprivate func handleFlagsChanged(flags: CGEventFlags, keyCode: UInt16) -> Bool {
        if TriggerConfig.sharedShiftGesturesEnabled,
           keyCode == KC.leftShift || keyCode == KC.rightShift {
            handleSharedShiftGesture(keyCode: keyCode)
            return false
        }
    ...
    private func handleSharedShiftGesture(keyCode: UInt16) {
        let side: ShiftSide = keyCode == KC.rightShift ? .right : .left
        // CGEventFlags has one shared mask for both Shift keys, so the keyCode
        // plus our observed set is the reliable per-side down/up signal.
        let isDown = !observedShiftSides.contains(side)
        if isDown {
            observedShiftSides.insert(side)
        } else {
            observedShiftSides.remove(side)
        }
        let actions = shiftGestures.shiftChanged(
            side: side,
            isDown: isDown,
            at: ProcessInfo.processInfo.systemUptime
        )
        performShiftActions(actions)
    }

    private func performShiftActions(_ actions: [ShiftGestureAction]) {
        for action in actions {
            switch action {
            case let .scheduleSingle(deadline):
                pendingShiftSingle?.cancel()
                let work = DispatchWorkItem { [weak self] in
                    guard let self else { return }
                    self.pendingShiftSingle = nil
                    let dueActions = self.shiftGestures.singleTapDeadlineReached(
                        at: ProcessInfo.processInfo.systemUptime
                    )
                    self.performShiftActions(dueActions)
                }
                pendingShiftSingle = work
            ...
            case .switchLayout:
                rslog("shared shift: switch layout")
                // The event tap runs on the main run loop. Calling directly is
                // intentional: a following keyDown must see the new layout.
                onSwitchHotkey?()
...
private func keyboardCallback(
    ...
    if event.getIntegerValueField(.eventSourceUserData) == kLocalSwitcherEventMarker {
        return Unmanaged.passRetained(event)
    }
