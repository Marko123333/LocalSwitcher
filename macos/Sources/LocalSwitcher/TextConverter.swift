...
final class TextConverter {
    ...
    nonisolated private let injectQueue = DispatchQueue(label: "com.marko.localswitcher.inject", qos: .userInteractive)
    ...
    nonisolated private func makeSource() -> CGEventSource? {
        ...
        source?.userData = kLocalSwitcherEventMarker
        return source
    }
    ...
    func convert(wordKeys: [TypedKey], prevWordKeys: [TypedKey], boundaryCount: Int,
                 passthroughSuffix: String = "", replacementOverride: String? = nil) -> Bool {
        let keys: [TypedKey]
        ...
        let converted = replacementOverride ?? pair.converted
    ...
    /// Replaces a buffered word without changing the active keyboard layout.
    /// Used for same-language typography such as е→ё and accidental capitals.
    func replace(
        wordKeys: [TypedKey],
        prevWordKeys: [TypedKey],
        boundaryCount: Int,
        ...
        guard !replacement.isEmpty,
              !isConverting,
              let pair = DynamicKeyMapping.convertKeys(keys),
              pair.original != replacement else { return false }

        isConverting = true
        let spaces = String(repeating: " ", count: trailingSpaces)
        let backspaces = keys.count + passthroughSuffix.count + trailingSpaces
        let insert = replacement + passthroughSuffix + spaces
        lastOriginal = pair.original + passthroughSuffix + spaces
        lastConverted = insert
        lastWasBuffer = true
        rslog("buffer polish: \(keys.count) keys (+\(passthroughSuffix.count) punct, +\(trailingSpaces) sp)")
